...
contract DataVault {
    ...
    function storeData(string memory _uri) public returns (uint256) {
        dataCounter++;
        // Initialize the Data struct and store it in the vault mapping
        vault[dataCounter] = Data({
            uri: _uri,
            owner: msg.sender,
            shared: false,
            sharedWith: new address  // Initialize with an empty array
        });
    ...
    function revokeAccess(uint256 _dataId, address _recipient) public onlyOwner(_dataId) {
        ...
        for (uint256 i = 0; i < vault[_dataId].sharedWith.length; i++) {
            if (vault[_dataId].sharedWith[i] == _recipient) {
                ...
                vault[_dataId].sharedWith[i] = vault[_dataId].sharedWith[vault[_dataId].sharedWith.length - 1];
                vault[_dataId].sharedWith.pop(); // Remove the last element
